...
export const parseTypeormEnv = env => {
	...
	if (TYPEORM_DATABASE_TYPE !== 'mysql') {
		throw TypeError(`env TYPEORM_DATABASE_TYPE expect mysql not '${TYPEORM_DATABASE_TYPE}'`);
	}

	const { TYPEORM_HOST } = env;

	if (!TYPEORM_HOST) {
		throw TypeError(`env TYPEORM_HOST expect not a falsy value but '${TYPEORM_HOST}'`);
	}

	const TYPEORM_PORT = parseInt(env.TYPEORM_PORT, 10);

	if (Number.isNaN(TYPEORM_PORT)) {
		throw TypeError(`env TYPEORM_PORT expect not NaN but '${TYPEORM_PORT}'`);
	}

	const { TYPEORM_DATABASE } = env;

	if (!TYPEORM_DATABASE) {
		throw TypeError(`env TYPEORM_DATABASE expect not a falsy value but '${TYPEORM_DATABASE}'`);
	}

	const { TYPEORM_USERNAME } = env;

	if (!TYPEORM_USERNAME) {
		throw TypeError(`env TYPEORM_USERNAME expect not a falsy value but '${TYPEORM_USERNAME}'`);
	}

	const { TYPEORM_PASSWORD } = env;

	if (!TYPEORM_PASSWORD) {
		throw TypeError(`env TYPEORM_PASSWORD expect not a falsy value but '${TYPEORM_PASSWORD}'`);
	}

	const TYPEORM_LOGGING = !!env.TYPEORM_LOGGING;

	if (!TYPEORM_LOGGING) {
		console.warn('env TYPEORM_LOGGING is not specified, use default value `false`');
	}

	const TYPEORM_SYNCHRONIZE = !!env.TYPEORM_SYNCHRONIZE;

	if (!TYPEORM_SYNCHRONIZE) {
		console.warn('env TYPEORM_SYNCHRONIZE is not specified, use default value `false`');
	}

	if (TYPEORM_SYNCHRONIZE === true) {
		console.warn('env TYPEORM_SYNCHRONIZE set true, will synchronize database schema');
	}
...
export const parseJWTEnv = (env: any) => {
	...
	if (!JWT_REFRESH_TOKEN_SECRET) {
		throw new TypeError(`JWT_REFRESH_TOKEN_SECRET should specified but not`);
	}
	...
	const JWT_ACCESS_TOKEN_DURATION = parseInt(env.JWT_ACCESS_TOKEN_DURATION, 10);

	if (Number.isNaN(JWT_ACCESS_TOKEN_DURATION)) {
		throw new TypeError(`JWT_ACCESS_TOKEN_DURATION should number but not`);
	}

	const JWT_REFRESH_TOKEN_DURATION = parseInt(env.JWT_REFRESH_TOKEN_DURATION, 10);
	...
	if (!JWT_REFRESH_TOKEN_COOKIE_KEY) {
		throw new TypeError(`JWT_REFRESH_TOKEN_COOKIE_KEY should specified but not`);
	}
